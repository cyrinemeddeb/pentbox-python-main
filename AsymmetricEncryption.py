...
class AsymmetricEncryption:
	...
	def encrypt(cls, public_key, elGamalKeys, key, method='RSA'):
		plaintext = pyip.inputStr('Entrer le message à crypter :  ')
		plaintext = str.encode(plaintext)

		if(method=='RSA'):
			choice = pyip.inputMenu(['Crypter le message', 'Signer le message'])
	...
	def decrypt(cls, key, elGamalKeys, method='RSA', choice='encrypt', signature=''):
		if(method=='RSA'):
			if(choice=='encrypt'):
				...
				msg = pyip.inputStr('Entrer le message original :  ')
	...
	def rsaVerifySignature(cls, msg, signature, key):
		...
		print("Signature valide:", hash == hashFromSignature)
	...
	def menu(cls):
		...
		ascii_banner = pyfiglet.figlet_format("CRYPTAGE DECRYPTAGE ASYMETRIQUE")
		print(ascii_banner)
		...
		while(True):
			...
			if(choice=='Crypter un message'):
				...
				print('\nVoulez-vous décrypter votre message ?  ')
