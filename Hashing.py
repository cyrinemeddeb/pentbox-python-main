...
class Hashing:
	...
	def password_cracker(cls, hash):
		...
		return 'Désolé, on n\'a pas trouvé le mot de passe'
	...
	def hash_menu(cls):
		...
		while(True):
			...
			if(choice=='Hacher un message'):
				text = pyip.inputStr('Entrer le message à hacher : \n')
				hashed_text = Hashing.multidigest(text)
				print('Le message haché est : ' + hashed_text)
			else:
				return
	...
	def crack_menu(cls):
		...
		while(True):
			...
			if(choice=='Cracker'):
				hash = pyip.inputStr('Entrer le message à cracker : \n')
				cracked_hash = Hashing.password_cracker(hash)
				print('Le message cracké est :  ' + cracked_hash)
			else:
				return